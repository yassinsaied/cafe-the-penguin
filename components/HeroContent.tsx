export default function HeroContent() {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-6 pt-10 pb-8 md:pt-12.5 md:pb-45">
      ...
      {/* ── Gold separator ── */}
      <div
        className="w-15 h-0.75 rounded mb-5"
        style={{ background: 'linear-gradient(to right, #3D1F00, #C9A84C, #3D1F00)' }}
      />

      {/* ── Description ── */}
      <p className="font-body font-light text-[15px] text-cream tracking-[0.04em] max-w-125 mb-12 leading-[1.7]">
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        ...
        <button
          className="font-heading font-medium text-sm tracking-[0.20em] uppercase text-espresso px-10 py-3.75 cursor-pointer transition-all duration-200 hover:opacity-90 hover:-translate-y-px"
          style={{
        ...
        <button className="font-heading font-medium text-sm tracking-[0.20em] uppercase text-gold-light border-2 border-gold bg-transparent px-9.5 py-3.25 cursor-pointer transition-all duration-200 hover:bg-gold hover:text-espresso hover:-translate-y-px">
          View Menu
        </button>
      </div>
    </div>
